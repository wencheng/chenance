 ...
 * <dd>(Most) GTK themes draw the Text border, rather than a border around the
 * whole combo, thus CCombo stands out as distinctly non-native - especially
 * GNOME's new standard "ClearLooks" which shows the gray background between
 * Text and Arrow and border</dd>
 * <dt>Arrow gets the focus:</dt>
 * <dd>It used to take two tabs to cross a CCombo. Solution: if the arrow gets
 * a SWT.FOCUS_IN event, ignore it and setFocus to the Text</dd>
...
//		text.setBounds(0, (int)Math.floor((height - text.computeSize(SWT.DEFAULT, SWT.DEFAULT, changed).y) / 2.0) , width - arrowSize.x, height);
	...
	public boolean setFocus() {
		checkWidget();
		// If a CellEditor is set to be activated by a double-click on the cell,
		// a 3-clicks will make (1)CellEditor activated, (2)setFocus() invoked,
		// (3)popup dropped down and (4)setFocus() invoked again.
		// Thus, popup will be closed after 3-clicks.
		if (this.isDropped())
			return popup.setFocus();
